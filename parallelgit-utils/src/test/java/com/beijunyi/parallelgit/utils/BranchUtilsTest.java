...
public class BranchUtilsTest extends AbstractParallelGitTest {
  ...
  public void getBranchHeadCommitTest() throws IOException {
    ...
    Assert.assertEquals(firstCommit, BranchUtils.getHeadCommit(Constants.MASTER, repo));
    writeToCache("a.txt");
    String branchName = "second";
    AnyObjectId branchCommit = commitToBranch(branchName, firstCommit);
    Assert.assertEquals(branchCommit, BranchUtils.getHeadCommit(branchName, repo));
  }
  ...
  public void resetBranchHeadTest() throws IOException {
    ...
    Assert.assertEquals(branchCommit, BranchUtils.getHeadCommit(Constants.MASTER, repo));
  }
